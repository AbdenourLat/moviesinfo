...
const Header = () => {
  let activeClass = "block py-2 pl-3 pr-4 text-white bg-blue-700 rounded md:bg-transparent md:text-blue-700 md:p-0 md:dark:text-blue-500";
  let notActive = "block py-2 pl-3 pr-4 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:hover:text-blue-700 md:p-0 md:dark:hover:text-blue-500 dark:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700";
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const destination = 'search?querry=' + e.target.search.value;
  ...
  return (
    <header>  
      <nav className="bg-white border-b-2 border-gray-200 dark:bg-gray-900 dark:border-gray-900 dark:border-b-1">
        <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        <Link to="/" className="flex items-center">
            <img src={Logo} className="h-8 mr-3" alt="Flowbite Logo" />
            <span className="self-center text-2xl font-semibold whitespace-nowrap dark:text-white">MoviesInfo</span>
        </Link>
        ...
        <button onClick={() => setDark(!dark)} data-tooltip-target="navbar-search-example-toggle-dark-mode-tooltip" type="button" data-toggle-dark="light" className="flex items-center p-2 mr-2 text-xs font-medium text-gray-700 bg-white rounded-lg border border-gray-200 toggle-dark-state-example hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-gray-300 dark:focus:ring-gray-500 dark:bg-gray-800 focus:outline-none dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">
          ...
          <div className={"items-center justify-between "+ (hidden ? "hidden" : "" ) +" w-full md:flex md:w-auto md:order-1"} id="navbar-search">
            ...
            <ul className="flex flex-col p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 lg:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
              <li>
                <NavLink to="/" className={({isActive}) => isActive ? activeClass : notActive } aria-current="page" end>Home</NavLink>
              </li>
              <li>
                <NavLink to="popular" className={({isActive}) => isActive ? activeClass : notActive } >Popular</NavLink>
              </li>
              <li>
                <NavLink to="top" className={({isActive}) => isActive ? activeClass : notActive }>Top Movies</NavLink>
              </li>
              <li>
                <NavLink to="upcoming" className={({isActive}) => isActive ? activeClass : notActive }>Upcoming</NavLink>
              </li>
            </ul>
          </div>
        </div>
